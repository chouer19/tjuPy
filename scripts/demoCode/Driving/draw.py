...
def readMap():
   ...
   mapMinX = 999999
   mapMinY = 9999999
   #f = open('./YBY-1.txt','r',encoding='utf-8', errors='ignore')
   f = open('./YBY-1.txt')
   lineRead = f.readline()
...
onlineData = []
class Point:
   def __init__(self,x,y,h,lat,lon):
      self.x = x
      self.y = y
      self.heading = h
      self.lat = lat
...
def readOfflineData():
   ...
   mapMinX = 999999
   mapMinY = 9999999
   #f = open('./065631.txt','r',encoding='utf-8', errors='ignore')
   #f = open('./065631.txt')
   f = open('./map')
   lineRead = f.readline()
   #for lineRead in open('../data/YBY-1.txt',encoding='utf-8', errors='ignore'):
   ...
   while lineRead:
      ...
      if True:
         ...
         x,y = utils.BLH2XYZ(lat,lon,heading)
         #print(x,'   ',y)
         mapMinX = x if x < mapMinX else mapMinX
         mapMinY = y if y < mapMinY else mapMinY
         point = Point(x,y,heading,lat,lon)
...
def drawOffDataInRoll(curIndex):
   global j
   j = (j + 1) % 9999
   content = {"Lat":onlineData[curIndex].lat,"Lon":onlineData[curIndex].lon,"Head":onlineData[curIndex].heading,"Speed":20}
   if j%10 == 0:
      prGreen(content)
   if content['Head'] > 360:
