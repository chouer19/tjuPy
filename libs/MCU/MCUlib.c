...
extern "C"  unsigned char *readBrake(){
    ...
    while(!haveBrake){//1.1
        ...
        for(int i=canLen - 1; i >=0; i--){//1.1.1
            if(brake_receive[i].ID == 0x411){//000
                haveBrake = 1;
                for(int j=0;j<8;j++){
...
extern "C"  void sendBrake(unsigned char control_mode, unsigned char control_dir,unsigned char control_depth, unsigned char control_time){
    CAN_OBJ send_brake[1];
    InitCANOBJ(send_brake,1);
    for(int i=0;i<1;i++){
        send_brake[0].ID = 0x510;
        send_brake[0].ExternFlag = 0;//4he0
        send_brake[0].RemoteFlag = 0;//2he0
        send_brake[0].DataLen = 8;
        send_brake[0].Data[0] = control_mode;
        send_brake[0].Data[1] = control_dir;
        send_brake[0].Data[2] = control_depth;
        send_brake[0].Data[3] = control_time;
...
extern "C"  void sendGun(unsigned char control_mode, unsigned char control_dir, unsigned char control_depth, unsigned char brake_light){
    CAN_OBJ send_gun[1];
    ...
    for(int i=0;i<1;i++){
        ...
        send_gun[0].Data[1] = control_dir;
        send_gun[0].Data[2] = control_depth;
        send_gun[0].Data[3] = brake_light;
...
extern "C"  void sendSteer(unsigned char control_mode, unsigned char steer_h, unsigned char steer_l,unsigned char calib){
    ...
    for(int i=0;i<1;i++){
        ...
        send_gun[0].Data[6] = 0x00;
        send_gun[0].Data[7] = control_mode ^ 0x74 ^ 0x94 ^ steer_h ^ steer_l ^ 0x00 ^ 0x00;
    }
    SendCanData(can250, send_gun, 1);
}
